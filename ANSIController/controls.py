...
class _ColorsControls:
    ...
    def __fix_skip(self,text:str,_found:list[str]) -> tuple[str,list[str]]:
        ...
        return (text,_found)
    def ___clear_escapes___(self,_found:list[str],_fsep:str,_esep:str) -> list:
        _founds = []
        for _f in _found:
            if _f.startswith(f"{_fsep}\\") and _f.endswith(f"\\{_esep}"):
                continue
            _founds.append(_f)
        return _founds
    def colorize(self,text:str,sep:str="[]") -> str:
        ...
        _fsep = re.escape(fsep)
        _esep = re.escape(esep)
        text = _convert_hex_colors(text)
        _re = fr"{_fsep}[^{_fsep}{_esep}].*?{_esep}"
        _found = re.compile(rf"({_re})").findall(text)
        _found = self.___clear_escapes___(_found,fsep,esep)
        text,_found = self.__fix_skip(text,_found)
        return self.__swap(text,_found).replace("[\\",'[').replace("\\]",']')
